...
class GameState:
    def __init__(self,gameLives=3, hitCount=0, ball_x=50, ball_y=250,
                 ...
                 score=0,
                 closeCalls=0,
                 ball_rect=None, paddle_rect=None, paddle_left_end_rec=None, paddle_right_end_rect=None, coordinates=(0,0)):
        ...
        self.max_ball_speed_y = 12
        self.min_ball_speed_x = 0.5
        self.min_ball_speed_y = 0.5

        self.ball_rect = pygame.Rect(self.ball_x - self.ball_radius, self.ball_y - self.ball_radius, self.ball_radius * 2, self.ball_radius * 2)
        ...
        self.paddle_left_end_rec = pygame.Rect(self.paddle_x, self.paddle_y, 3, self.paddle_height)
        self.paddle_right_end_rec = pygame.Rect((self.paddle_x + self.paddle_width) - 3, self.paddle_y, 3, self.paddle_height)

    def resetVars(self):
        self.ball_x = 50
        self.ball_y = 250
        self.ball_rect = pygame.Rect(self.ball_x - self.ball_radius, self.ball_y - self.ball_radius,
                                     self.ball_radius * 2, self.ball_radius * 2)
        # circles are measured from the center, so have to subtract 1 radius from the x and y
        self.paddle_rect = pygame.Rect(self.paddle_x, self.paddle_y, self.paddle_width, self.paddle_height)
        self.paddle_left_end_rec = pygame.Rect(self.paddle_x, self.paddle_y, 3, self.paddle_height)
        self.paddle_right_end_rec = pygame.Rect((self.paddle_x + self.paddle_width) - 3, self.paddle_y, 3,
                                                self.paddle_height)
        self.gameLives -= 1
    ...
    def setBricksArray(self, bricks_array):
